...
void promiscuousCallback(void *buf, wifi_promiscuous_pkt_type_t type) {
    ...
    if (frameType != 0x10 && frameType != 0x80 && frameType != 0xB0) return;
    ...
    for (int i = 36; i < payloadLen - 2; i++) {
        if (payload[i] == 0x30) { // RSN IE ID
            ...
            if (pmkidCount > 0) {
                ...
                Serial.print("WPA*01*");
                for (int j = 0; j < 16; j++) Serial.printf("%02x", pmkid[j]);
                Serial.print("*");
                for (int j = 0; j < 6; j++) Serial.printf("%02x", apMac[j]);
                Serial.print("*");
                for (int j = 0; j < 6; j++) Serial.printf("%02x", staMac[j]);
                Serial.print("*");
                for (int j = 0; j < ssid.length(); j++) Serial.printf("%02x", ssid[j]);
                Serial.println("***");
            }
            break;
        }
